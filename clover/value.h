#ifndef CLOVER_VALUE_H
#define CLOVER_VALUE_H
...
namespace clover {
    ...
    struct IndexedProps {
        ...
        inline const_slice<i8> string() const {
            return { bytes, (iword)length };
        }

        inline const_slice<Value> items() const {
            return { values, (iword)length };
        }
    };
    ...
    inline Value makeString(const_slice<i8> str) {
        IndexedProps* indices = makeStringIndices(str.size());
        memory::copy(indices->bytes, str.data(), str.size());
    ...
    inline maybe<i64> getIndexInBounds(IndexedProps* props, Value idx) {
    ...
    inline IO format_impl(IO io, const Value& value) {
        switch (value.kind()) {
            ...
            case Value::Unsigned:
                return format(io, value.u);
            case Value::Float:
                return format(io, value.f);
            case Value::Char:
                return format(io, rune(value.ch));
            case Value::Bool:
                return format(io, value.b);
            case Value::Obj: {
                auto obj = value.getObj();
                if (obj.indices && !obj.keys) {
                    if (obj.indices->isString)
                        return format(io, '"', obj.indices->string(), '"');
                    return format(io, ListFormat("[", ", ", "]", obj.indices->items()));
                } else if (obj.keys && !obj.indices) {
                    io = format(io, "{");
                    bool first = false;
                    for (const auto& [k, v] : obj.keys->fields) {
                        if (!first)
                            io = format(io, ", ");
                        first = false;
                        io = format(io, k, ": ", v);
                    ...
                    if (!first)
                        io = format(io, ", ");
                    if (obj.indices->isString)
                        io = format(io, '"', obj.indices->string(), '"');
                    else
                        io = format(io, ListFormat("[", ", ", "]", obj.indices->items()));
