...
LDFLAGS := -Wl,--gc-sections -O3 -flto
ASM := as
ASMFLAGS := 
...
ifeq ($(OS),Windows_NT)
	...
	ARCHSTR := $(shell uname -p | tr [:upper:] [:lower:])
...
CORE_ASMS := $(wildcard core/native/sys_$(OSSTR)_$(ARCHSTR).s)
CORE_NATIVE_OBJS := $(CORE_ASMS:.s=.o)

main: release
release: clover jasmine libcore libcclover
